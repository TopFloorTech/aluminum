...
abstract class AluminumBlockBase extends BlockBase implements BlockPluginInterface
{
    ...
    public function getAluminumId() {
      ...
      return strtolower(preg_replace(['/([a-z\d])([A-Z])/', '/([^_])([A-Z][a-z])/'], '$1_$2', self::class));
    }
    ...
    public function getOptionValue($option_name, $replace_tokens = FALSE) {
        ...
        $value = isset($config[$option_name]) ? $config[$option_name] :
            isset($options[$option_name]['#default_value']) ? $options[$option_name]['#default_value'] : '';
    ...
    public function blockForm($form, FormStateInterface $form_state) {
        ...
        foreach ($this->getOptions() as $option_name => $option) {
            ...
            $option['#default_value'] = isset($config[$option_name]) ? $config[$option_name] :
                isset($option['#default_value']) ? $option['#default_value'] : '';
    ...
    {
        $default_config = \Drupal::config($this->getAluminumId() . '.settings');
